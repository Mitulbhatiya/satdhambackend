const YoutubeLink = require("../../../models/sat/uploadYoutubeLink")
...
const getYoutubeLink = async (req, res, next) => {
    try {
        const youtube = await YoutubeLink.find({ folderId: req.body.folderId });
        res.status(200).json({
...
const updateYoutubeLink = async (req, res, next) => {
    try {
        const body = req.body
        await YoutubeLink.updateOne({ _id: String(body._id) }, { $set: { link: String(body.link) } })
            .then((ress) => {
                res.status(200).json({

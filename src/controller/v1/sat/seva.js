const Seva = require("../../../models/sat/seva")
                ...
                await s3.send(command)
                    .then(async (ress1) => {
                        ...
                        await Promise.all(req.files?.map(async data => {
                            ...
                            try {
                                const responseUpdate = await Seva.updateOne(
                                    { _id: String(req.body._id) },
                                    { $set: { fileName: `SAT_${Date.now()}_${Math.random().toString(36).substring(7)}.png` } }
                                )
                                if (responseUpdate) {
                                    const ress = await Seva.findOne({ _id: String(req.body._id) })
                                    if (ress) {
...
const deleteUpload = async (req, res, next) => {
    try {
        ...
        await s3.send(command)
            .then(async (ress1) => {
                Seva.deleteOne({ _id: String(req.body._id) })
                    .then((result) => {
                        res.status(200).json({
                            message: "Deleted!",
                            err: result

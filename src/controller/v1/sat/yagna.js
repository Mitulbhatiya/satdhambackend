const Yagna = require("../../../models/sat/yagna")
            ...
            if (err) {
                ...
                await Promise.all(req.files?.map(async data => {
                    ...
                    try {
                        ...
                        if (ress) {
                            const fileUpload = bucket.file(`yagna/${ress.fileName}`);
...
const getUpload = async (req, res, next) => {
    try {
        const uploads = await Yagna.find({});
        res.status(200).json({
...
const updateUpload = async (req, res, next) => {
    ...
    try {
        await upload(req, res, async function (err) {
            ...
            try {
                const oldFile = bucket.file(`yagna/${req.body.fileName}`);
            ...
            await Promise.all(files.map(async data => {
                try {
                    const newFileName = `SAT_${Date.now()}_${Math.random().toString(36).substring(7)}.png`;

                    const responseUpdate = await Yagna.updateOne(
                    ...
                    const record = await Yagna.findOne({ _id: String(req.body._id) });
                    if (!record) {
                        errorCount++;
                        return;
                    }

                    const fileUpload = bucket.file(`yagna/${record.fileName}`);
                    await fileUpload.save(data.buffer, {
                        metadata: { contentType: data.mimetype },
                        public: true,
                        resumable: false,
...
const deleteUpload = async (req, res, next) => {
    try {
        const bucket = await getBucket();
        const filePath = `yagna/${req.body.fileName}`;
        ...
        await Yagna.deleteOne({ _id: String(req.body._id) })
            .then((result) => {
                res.status(200).json({
                    message: "Deleted!",
                    err: result

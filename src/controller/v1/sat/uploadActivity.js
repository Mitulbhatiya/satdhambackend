const Folder = require("../../../models/sat/folderActivity")
const UploadFile = require("../../../models/sat/activityUpload")
    ...
    try {
        await upload(req, res, async function (err) {
            ...
            await Promise.all(files.map(async (data) => {
                try {
                    const newFileName = `ACTIVITY_${Date.now()}_${Math.random().toString(36).substring(7)}.png`;
                    ...
                    const fileUpload = bucket.file(`activity/${record.fileName}`);
                    await fileUpload.save(data.buffer, {
                        metadata: { contentType: data.mimetype },
                        public: true,
                        resumable: false,

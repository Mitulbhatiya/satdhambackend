...
const Whqr = require("../../../models/sat/whqr")
        ...
        await upload(req, res, async function (err) {
            ...
            if (err) {
                ...
                await Promise.all(req.files?.map(async data => {
                    ...
                    const newFile = new Whqr({
                        fileName: `WHQR_${Date.now()}_${Math.random().toString(36).substring(7)}.png`
                    });
                ...
                await s3.send(command)
                    .then(async (ress1) => {
                        ...
                        await Promise.all(req.files?.map(async data => {
                            ...
                            try {
                                await Whqr.updateOne(
                                    { _id: String(req.body._id) },
                                    { $set: { fileName: `WHQR_${Date.now()}_${Math.random().toString(36).substring(7)}.png` } }
                                )
                                if (Whqr) {
                                    const ress = await Whqr.findOne({ _id: String(req.body._id) })
                                    if (ress) {
...
const deleteUpload = async (req, res, next) => {
    try {
        ...
        await s3.send(command)
            .then(async (ress1) => {
                Whqr.deleteOne({ _id: String(req.body._id) })
                    .then((result) => {
                        res.status(200).json({
                            message: "Deleted!",
                            err: result

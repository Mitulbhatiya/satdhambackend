        ...
        await upload(req, res, async function (err) {
            ...
            if (err) {
                ...
                await Promise.all(req.files?.map(async data => {
                    ...
                    const newFile = new DailyDarshan({
                        fileName: `DD_${Date.now()}_${Math.random().toString(36).substring(7)}.png`,
...
const getUpload = async (req, res, next) => {
    try {
        ...
        const uploads = await DailyDarshan.find({ location: String(location) })
        console.log(uploads);
        res.status(200).json({

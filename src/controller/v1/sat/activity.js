...
const Activity = require("../../../models/sat/activity")
        ...
        await upload(req, res, async function (err) {
            ...
            if (err) {
                ...
                await Promise.all(req.files?.map(async data => {
                    ...
                    const newFile = new Activity({
                        fileName: `ACTIVITY_${Date.now()}_${Math.random().toString(36).substring(7)}.png`,
                    ...
                    try {
                        ...
                        if (ress) {
                            const fileUpload = bucket.file(`activity/${ress.fileName}`);
...
const getUpload = async (req, res, next) => {
    try {
        ...
        const uploads = await Activity.find({ location: String(location) });
        res.status(200).json({
    ...
    try {
        await upload(req, res, async function (err) {
            ...
            if (err || req.files.length > 1) {
                ...
                try {
                    const oldFile = bucket.file(`activity/${req.body.fileName}`);
                    ...
                    // const parms = {
                    //     Bucket: bucket_name,
                    //     Key: `activity/${req.body.fileName}`,
                    // }
                ...
                await Promise.all(req.files?.map(async data => {
                    ...
                    try {
                        await Activity.updateOne(
                            ...
                            { $set: { fileName: `ACTIVITY_${Date.now()}_${Math.random().toString(36).substring(7)}.png`, location: String(req.body.location) } }
                        )
                        const ress = await Activity.findOne({ _id: String(req.body._id) })
                        if (ress) {
...
const deleteUpload = async (req, res, next) => {
    try {
        const bucket = await getBucket();
        const filePath = `activity/${req.body.fileName}`;
        ...
        Activity.deleteOne({ _id: String(req.body._id) })
            .then((result) => {
                res.status(200).json({
                    message: "Deleted!",
                    err: result

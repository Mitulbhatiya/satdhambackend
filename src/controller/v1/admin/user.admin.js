...
const userData = async (req, res, next) => {
    try {
        await Admin.findOne({ _id: req.userData.userId }, { password: 0, createdAt: 0, updatedAt: 0 })
            .then((result) => {
                if (result !== null) {
                    res.status(200).json({
                        message: "success",
...
const changePassword = async (req, res, next) => {
    try {
        ...
        await Admin.findOne({ _id: req.userData.userId })
            .then((result) => {
                if (!result) {
                ...
                if (checkPasswordStatus !== true) {
                    ...
                    Admin.updateOne({ _id: result._id }, { $set: { password: hash } })
                        .then((result1) => {
